...
var margin;
// let colorbg = "e7ddb1".split("-").map((a) => "#" + a + "80");
let colorbg = "f9f7eb".split("-").map((a) => "#" + a + "80");
// let colorbg2 = "f8f1d3".split("-").map((a) => "#" + a + "80");
let colorbg2 = "ffffff".split("-").map((a) => "#" + a + "80");
let filter, grad;
//moutain
var xOff, yOff;
var color1, color2;
let colorsA = "e3d7a3BF-f5989d33-ea618e40-fce4d6b3-caac7180".split("-").map((a) => "#" + a);
let colorsA2 = "e3d7a3-f5989d-ea618e-fce4d6-caac71".split("-").map((a) => "#" + a + "00");
let colorsB = "6cbbd4BF-5863b140-5282a1BF-fee8f733-c1f29b80".split("-").map((a) => "#" + a);
let colorsB2 = "6cbbd4-5863b1-5282a1-fee8f7-c1f29b".split("-").map((a) => "#" + a + "00");
let colorsC = "1b1b1bBF-29292940-f3f3f3BF-222222b3-ff000080".split("-").map((a) => "#" + a);
let colorsC2 = "1b1b1b-292929-f3f3f3-222222-ff0000".split("-").map((a) => "#" + a + "00");
let colorsD = "70c1b3BF-aaf0d340-50514f33-ffe066b3-247ba080".split("-").map((a) => "#" + a);
let colorsD2 = "70c1b3-aaf0d3-50514f-ffe066-247ba0".split("-").map((a) => "#" + a + "00");
let colorsE = "3c1642BF-38637540-08637533-b2ff9eb3-1dd3b080".split("-").map((a) => "#" + a);
let colorsE2 = "3c1642-386375-086375-b2ff9e-1dd3b0".split("-").map((a) => "#" + a + "00");
let colorbgA = "ffffff".split("-").map((a) => "#" + a);
let colorbgA2 = "ffffff07".split("-").map((a) => "#" + a);
//river
let colorsR = "7fdeea-a3b7f0-a1e4f7-6d7db6-5a6696".split("-").map((a) => "#" + a);
let colorsR2 = "7fdeea-a3b7f0-a1e4f7-6d7db6-5a6696".split("-").map((a) => "#" + a + "00");
...
function setup() {
  ...
  hatDecLineCanvas = createGraphics(212, 105);
  ...
  clotheCanvas = createGraphics(228, 113);
  ...
  colorGroup.set("eyebrows", color(50, 50, 50));
  colorGroup.set("mouth", color(150, 150, 150));
  colorGroup.set("eye", color(50, 50, 50));
  colorGroup.set("center", color(176, 56, 60));
  colorGroup.set("black", color(50, 50, 50));
  colorGroup.set("shadow", "rgba(0, 0, 0, 0.5)");
  // random color 
  // colorGroup.set("main", color(random(255), random(255), random(255)));
  // colorGroup.set("dec", color(random(255), random(255), random(255)));
  // colorGroup.set("hat", color(random(255), random(255), random(255)));
  // colorGroup.set("ball", color(random(255), random(255), random(255)));
  // colorGroup.set("vice", color(random(255), random(255), random(255)));
  // random color - safeColor
  // colorGroup.set("main", color(safeColor.random()));
  // colorGroup.set("dec", color(safeColor.random()));
  // colorGroup.set("hat", color(safeColor.random()));
  // colorGroup.set("ball", color(safeColor.random()));
  // colorGroup.set("vice", color(safeColor.random()));
  // random color - ColourLovers
  // colorPalette = colourLovers[parseInt(random(200))];
  // colorPalette = shuffleArray(colorPalette);
  // colorPalette = shuffleArray(colorPalette);
  // colorGroup.set("main", colorPalette[0]);
  // colorGroup.set("dec", colorPalette[1]);
  // colorGroup.set("hat", colorPalette[2]);
  // colorGroup.set("ball", colorPalette[3]);
  // colorGroup.set("vice", colorPalette[4]);
  // random color - chinaTraditionalColor
  colorPalette = chineseTraditionalColor[parseInt(random(chineseTraditionalColor.length))];
  colorPalette = shuffleArray(colorPalette);
  colorPalette = shuffleArray(colorPalette);
  colorGroup.set("main", colorPalette[0]);
  colorGroup.set("dec", colorPalette[1]);
  ...
  colorGroup.set("ball", colorPalette[3]);
  colorGroup.set("vice", colorPalette[4]);
  // console.log('[['+colorPalette[0]+'], ['+colorPalette[1]+'], ['+colorPalette[2]+'], ['+colorPalette[3]+'], ['+colorPalette[4]+']]');
  // 2d->3d
  drawingContext.shadowOffsetX = 0;
  drawingContext.shadowOffsetY = 3;
  drawingContext.shadowBlur = 20;
  drawingContext.shadowColor = colorGroup.get("shadow");
  // background
	randomSeed(seed);
	color1 = random([colorsA, colorsB, colorsC, colorsD, colorsE]);
...
function draw() {
  ...
  // radial gradiant  
  // gradient = drawingContext.createRadialGradient(width/2, height*0.6, canvas*0.1, width/2, height*0.5, canvas*0.5);
  // gradient.addColorStop(0.1, colorGroup.get("bgcenter"));
  // gradient.addColorStop(0.6, colorGroup.get("background"));
  // gradient.addColorStop(0.8, colorGroup.get("background"));
  // gradient.addColorStop(1, colorGroup.get("background"));
  ...
  // mountain and river
	...
	//mountain and river
	noStroke();
	let mountain_h = height / int(random(10, 30));
	for (let n = 0; n < height; n += mountain_h) {
		...
		if (n > height / 3) {
			...
			beginShape();
			curveVertex(-n, n);
			for (let i = xOff; i < width + 100; i += 50) {
				let p = random(-50, 50);
				curveVertex(i, sin(i) * p);
			}
			...
			//river
			grad = drawingContext.createLinearGradient(0, -mountain_h, 0, mountain_h);
			grad.addColorStop(0, random(colorsR));
			grad.addColorStop(1, random(colorsR2));
			drawingContext.fillStyle = grad;
			beginShape();
			curveVertex(-n, n);
			for (let i = xOff; i < width - xOff; i += 200) {
				let p = random(-1, 1);
				curveVertex(i, cos(i) * p * random(50));
			}
			curveVertex(width + n, n);
			endShape(CLOSE);
		}
...
function drawFace() {
  ...
  faceSilhouette();
  ctx.strokeStyle = colorGroup.get("stroke");
  ctx.clip();
...
function drawHatDecBallLeft() {
  ...
  push();
  gradient = drawingContext.createLinearGradient(-hatDecBallLeftCanvas.width/3, -hatDecBallLeftCanvas.height/3, hatDecBallLeftCanvas.width*0.9, hatDecBallLeftCanvas.height*0.8);
  gradient.addColorStop(0, "rgb("+colorGroup.get("ball").map(color => color+80)+")");
  gradient.addColorStop(1, "rgb("+colorGroup.get("ball")+")");
  hatDecBallLeftCanvas.drawingContext.fillStyle = gradient;
...
function drawHatDecBallMiddle() {
  let distance = -107;
  push();
  gradient = drawingContext.createLinearGradient(-hatDecBallMiddleCanvas.width/3, -hatDecBallMiddleCanvas.height/3, hatDecBallMiddleCanvas.width*0.9, hatDecBallMiddleCanvas.height*0.8);
  gradient.addColorStop(0, "rgb("+colorGroup.get("ball").map(color => color+100)+")");
  gradient.addColorStop(1, "rgb("+colorGroup.get("ball")+")");
  hatDecBallMiddleCanvas.drawingContext.fillStyle = gradient;
...
function drawHatDecBallRight() {
  ...
  push();
  gradient = drawingContext.createLinearGradient(-hatDecBallRightCanvas.width/3, -hatDecBallRightCanvas.height/3, hatDecBallRightCanvas.width*0.9, hatDecBallRightCanvas.height*0.8);
  gradient.addColorStop(0, "rgb("+colorGroup.get("ball").map(color => color+80)+")");
  gradient.addColorStop(1, "rgb("+colorGroup.get("ball")+")");
  hatDecBallRightCanvas.drawingContext.fillStyle = gradient;
...
function drawShoes() {
  ...
  shoesSilhouette();
  ctx.clip();
  shoesCanvas.background(colorGroup.get("black"));
  pop();
...
shoesCanvas.stroke(colorGroup.get("black"));
...
function drawEyebrowsCenter() {
  ...
  ctx.clip();
  eyebrowsCenterCanvas.background(colorGroup.get("center"));
  pop();
  ...
  function eyebrowsCenterSilhouette() {
    ...
    eyebrowsCenterCanvas.stroke(colorGroup.get("center"));
...
function makeFilter() {
  ...
  push();
	...
	drawingContext.shadowColor = color(48, 33, 76, 10);
		...
		for (var j = 0; j < height; j++) {
			overAllTexture.set(
				...
				color(
					...
					8,
					100,
					noise(i / 3, j / 3, (i * j) / 10) * random(55, 75)
				)
			);
		}
	}
